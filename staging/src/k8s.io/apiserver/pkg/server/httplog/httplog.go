...
func withLogging(handler http.Handler, stackTracePred StacktracePred, shouldLogRequest ShouldLogRequestPred) http.Handler {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0354")
...
func (rl *respLogger) Log() {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0355")
...
func (rl *respLogger) Header() http.Header {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0356")
	return rl.w.Header()
}

// Write implements http.ResponseWriter.
func (rl *respLogger) Write(b []byte) (int, error) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0357")
...
func (rl *respLogger) WriteHeader(status int) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0358")
