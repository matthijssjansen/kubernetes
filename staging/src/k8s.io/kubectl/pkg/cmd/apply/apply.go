...
func NewCmdApply(baseName string, f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	klog.Infof("%s [CONTINUUM] 0401 - KUBECTL CREATE APPLY COMMAND")
...
func (flags *ApplyFlags) ToOptions(cmd *cobra.Command, baseName string, args []string) (*ApplyOptions, error) {
	klog.Infof("%s [CONTINUUM] 0403 - KUBECTL TO OPTIONS")
	if len(args) != 0 {
	...
	klog.Infof("%s [CONTINUUM] 0404 - KUBECTL TO OPTIONS END")
	return o, nil
}

// Validate verifies if ApplyOptions are valid and without conflicts.
func (o *ApplyOptions) Validate() error {
	klog.Infof("%s [CONTINUUM] 0405 - KUBECTL APPLY START VALIDATE")
...
func (o *ApplyOptions) Run() error {
	klog.Infof("%s [CONTINUUM] 0406 - KUBECTL APPLY START EXECUTE")
	...
	klog.Infof("%s [CONTINUUM] 0409 - KUBECTL APPLY END")
	return nil
}

func (o *ApplyOptions) applyOneObject(info *resource.Info) error {
	klog.Infof("%s [CONTINUUM] 0407 - KUBECTL APPLY ONE OBJECT")
...
See https://kubernetes.io/docs/reference/using-api/server-side-apply/#conflicts`, err)
	...
	if err := info.Get(); err != nil {
		...
		if o.DryRunStrategy != cmdutil.DryRunClient {
			...
			klog.Infof("%s [CONTINUUM] 0408 - KUBECTL APPLY CREATE AND SEND")
