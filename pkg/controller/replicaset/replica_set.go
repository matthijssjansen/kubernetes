...
func NewReplicaSetController(rsInformer appsinformers.ReplicaSetInformer, podInformer coreinformers.PodInformer, kubeClient clientset.Interface, burstReplicas int) *ReplicaSetController {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0164")
...
func NewBaseController(rsInformer appsinformers.ReplicaSetInformer, podInformer coreinformers.PodInformer, kubeClient clientset.Interface, burstReplicas int,
	...
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0165")
...
func (rsc *ReplicaSetController) Run(ctx context.Context, workers int) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0166")
...
func (rsc *ReplicaSetController) getReplicaSetsWithSameController(rs *apps.ReplicaSet) []*apps.ReplicaSet {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0167")
	controllerRef := metav1.GetControllerOf(rs)
	if controllerRef == nil {
...
func (rsc *ReplicaSetController) getPodReplicaSets(pod *v1.Pod) []*apps.ReplicaSet {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0168")
...
func (rsc *ReplicaSetController) resolveControllerRef(namespace string, controllerRef *metav1.OwnerReference) *apps.ReplicaSet {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0169")
...
func (rsc *ReplicaSetController) enqueueRS(rs *apps.ReplicaSet) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0170")
...
func (rsc *ReplicaSetController) enqueueRSAfter(rs *apps.ReplicaSet, duration time.Duration) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0171")
	key, err := controller.KeyFunc(rs)
	if err != nil {
		utilruntime.HandleError(fmt.Errorf("couldn't get key for object %#v: %v", rs, err))
		return
...
func (rsc *ReplicaSetController) addRS(obj interface{}) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0172")
...
func (rsc *ReplicaSetController) updateRS(old, cur interface{}) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0173")
...
func (rsc *ReplicaSetController) deleteRS(obj interface{}) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0174")
	rs, ok := obj.(*apps.ReplicaSet)
	if !ok {
...
func (rsc *ReplicaSetController) addPod(obj interface{}) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0163")
	pod := obj.(*v1.Pod)
...
func (rsc *ReplicaSetController) updatePod(old, cur interface{}) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0175")
	curPod := cur.(*v1.Pod)
	oldPod := old.(*v1.Pod)
...
func (rsc *ReplicaSetController) deletePod(obj interface{}) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0176")
	pod, ok := obj.(*v1.Pod)
...
func (rsc *ReplicaSetController) worker(ctx context.Context) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0177")
	for rsc.processNextWorkItem(ctx) {
	}
}

func (rsc *ReplicaSetController) processNextWorkItem(ctx context.Context) bool {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0178")
...
func (rsc *ReplicaSetController) manageReplicas(ctx context.Context, filteredPods []*v1.Pod, rs *apps.ReplicaSet) error {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0179")
...
func (rsc *ReplicaSetController) syncReplicaSet(ctx context.Context, key string) error {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0180")
	startTime := time.Now()
	defer func() {
...
func (rsc *ReplicaSetController) claimPods(ctx context.Context, rs *apps.ReplicaSet, selector labels.Selector, filteredPods []*v1.Pod) ([]*v1.Pod, error) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0181")
...
func slowStartBatch(count int, initialBatchSize int, fn func() error) (int, error) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0182")
	remaining := count
	successes := 0
...
func (rsc *ReplicaSetController) getIndirectlyRelatedPods(rs *apps.ReplicaSet) ([]*v1.Pod, error) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0183")
...
func getPodsToDelete(filteredPods, relatedPods []*v1.Pod, diff int) []*v1.Pod {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0184")
...
func reportSortingDeletionAgeRatioMetric(filteredPods []*v1.Pod, diff int) {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0185")
	now := time.Now()
...
func getPodsRankedByRelatedPodsOnSameNode(podsToRank, relatedPods []*v1.Pod) controller.ActivePodsWithRanks {
	fmt.Println(time.Now().UnixNano(), "[CONTINUUM] 0186")
